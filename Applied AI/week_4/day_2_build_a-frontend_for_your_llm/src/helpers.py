...
def init_chroma(collection_name: str, task: str = "retrieve"):
    ...
    try:
        if use_server:
            ...
            port = int(os.getenv("CHROMA_SERVER_HTTP_PORT", "8001"))
            ssl = os.getenv("CHROMADB_SSL", "false").lower() in ("1", "true", "yes")
            
            if ssl:
                chroma_client = chromadb.HttpClient(host=host, port=port, ssl=True)
            else:
